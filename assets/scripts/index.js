...
async function loadgallery(remainItemCount) {
    ...
    const node = createEleByTemp('gallery-item', {
        id: `#${items[displayedItemCount].match(
            /pic\/(.+)\.(jpg|png|jpeg|jfif|webp|gif)/
        )[1]
            }`,
...
function view() {
    ...
    for (const i of items) {
        if (
            i.search(new RegExp(`pic/${name}\.(jpg|png|jpeg|jfif|webp|gif)`)) != -1
        ) {
            name = i
            break
        }
...
; (async () => {
    ...
    if (development) {
        for (const i of domParser
            .parseFromString((await get('../pic/')).response, 'text/html')
            .querySelectorAll('#files a.icon-image')) {
            items.push(
                'pic/' +
                decodeURIComponent(
                    i.href.match(
                        /(?<=pic\/).+\.(jpg|png|jfif|webp|gif)/
                    )[0]
                )
            )
        }
